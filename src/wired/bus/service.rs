...
use lnpbp::lnp::application::peer::Peer;

use super::*;
use crate::msgbus::*;
use crate::wired::PeerConnectionList;
use crate::{BootstrapError, TryService};
...
impl BusService {
    pub fn init(
        ...
        trace!(
            "Subscribing on message bus requests on {} ...",
            config.socket_addr
        );
        ...
        subscriber
            .connect(config.socket_addr.as_str())
            .map_err(|e| BootstrapError::SubscriptionError(e))?;
    ...
    async fn command_connect(&mut self, connect: Connect) -> Result<Command, Error> {
        ...
        let peer = Peer::new_outbound(connect.node_addr, &node_secret, &ephemeral_secret).await?;
        trace!(
            "Connection to peer {} completed successfully",
            connect.node_addr
        );
